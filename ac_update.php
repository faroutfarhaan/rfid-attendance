...
if (isset($_POST['update'])) {
    ...
    elseif (!filter_var($up_email,FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z 0-9]*$/", $up_name)) {
        header("location: main.php?error=invalidEN&UN=".$up_name);
        exit();
    }
    ...
    elseif (!preg_match("/^[a-zA-Z 0-9]*$/", $up_name)) {
        header("location: main.php?error=invalidName&E=".$up_email);
        exit();
    }
    else{
        ...
        if ( !mysqli_stmt_prepare($result, $sql)){
            header("location: main.php?error=sqlerror1");
            exit();
        }
        else{
            ...
            if ($row = mysqli_fetch_assoc($resultl)) {
                ...
                if ($pwdCheck == false) {
                    header("location: main.php?error=wrongpasswordup");
                    exit();
                }
                    ...
                    if ($useremail == $up_email) {
                        ...
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                            ...
                            header("location: main.php?success=updated");
                            exit();
                        }
                    }
                    else{
                        ...
                        if ( !mysqli_stmt_prepare($result, $sql)){
                            ...
                            if (!$row = mysqli_fetch_assoc($resultl)) {
                                ...
                                header("location: main.php?error=nouser2");
                                exit();
                            }
                        }
                    }
                ...
                header("location: main.php?error=nouser1");
                exit();
            }
        }
    }
